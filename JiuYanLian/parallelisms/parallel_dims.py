...
class ParallelDims:
    ...
    def _validate(self):
        ...
        for d in (dp_replicate, cp, tp, pp):
            assert d >= 1, f"Parallelism degree should be >= 1, except for dp_shard. (dp_replicate, cp, tp, pp):{(dp_replicate, cp, tp, pp)}"
        ...
        assert dp_shard >= 1, (self.world_size, dp_replicate, cp, tp, pp)
        assert cp >= 1, cp
        assert tp >= 1, tp
        assert pp >= 1, pp
        ...
        logger.info(f"_validate dims dp_replicate({self.dp_replicate}) * dp_shard({self.dp_shard}) * cp({cp}) * tp({tp}) * pp({pp}) == WORLD_SIZE({self.world_size}) ")
