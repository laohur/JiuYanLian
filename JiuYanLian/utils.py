...
def get_num_params(model: torch.nn.Module, exclude_embedding: bool = False) -> int:
    ...
    if exclude_embedding:
        embed_tokens = getattr(model, "tok_embeddings", model.model.embed_tokens)
        num_params -= embed_tokens.weight.numel()
    return num_params

def get_num_flop_per_token(num_params: int, model_config, seq_len) -> int:
    n_layers = getattr(model_config, "n_layers", model_config.num_hidden_layers)
    n_heads = getattr(model_config, "n_heads", model_config.num_attention_heads)
    dim = getattr(model_config, "dim", model_config.hidden_size)
    l, h, q, t = (n_layers, n_heads, dim//n_heads, seq_len,)
    # l, h, q, t = (
