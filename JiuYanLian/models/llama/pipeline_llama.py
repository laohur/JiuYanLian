...
from JiuYanLian.models.llama.modeling_llama import ModelArgs
...
def pipeline_llama_manual_split(
    ...
    def _build_stage(stage_idx, start_layer, stop_layer, is_first=False, is_last=False):
        ...
        for name in list(model.layers.keys()):
            ...
            #     drop_layers = True
            # if drop_layers:
            if start_layer<=int(name)<stop_layer:
                continue
    ...
    for stage_idx in stage_ids_this_rank(pp_rank, pp_size, num_stages, style="loop"):
        start_layer = splits[stage_idx - 1] if stage_idx > 0 else 0
        stop_layer = splits[stage_idx] if stage_idx < num_stages - 1 else len(whole_model.layers)
